...
export async function runCriticalTests(): Promise<boolean> {
  ...
  results.push(await runTest('Arrondi 49.99€', () => {
    const cents = eurosToCents(49.99);
    if (cents !== 4999) throw new Error(`Expected 4999, got ${cents}`);
  }));
  
  results.push(await runTest('Arrondi 49.999€', () => {
    const cents = eurosToCents(49.999);
    if (cents !== 5000) throw new Error(`Expected 5000, got ${cents}`);
  }));
  ...
  results.push(await runTest('Montant avocat valide (49€)', () => {
    const result = validateAmount(49, 'lawyer');
    if (!result.valid) throw new Error(result.error);
  }));
  
  results.push(await runTest('Montant expat valide (19€)', () => {
    const result = validateAmount(19, 'expat');
    if (!result.valid) throw new Error(result.error);
  }));
  
  results.push(await runTest('Montant trop bas (3€)', () => {
    const result = validateAmount(3, 'lawyer');
    if (result.valid) throw new Error('Should reject amount below minimum');
  }));
  
  results.push(await runTest('Montant trop élevé (600€)', () => {
    const result = validateAmount(600, 'expat');
    if (result.valid) throw new Error('Should reject amount above maximum');
  }));
  
  results.push(await runTest('Montant avec warning (55€ pour avocat)', () => {
    const result = validateAmount(55, 'lawyer');
  ...
  results.push(await runTest('Répartition avocat 49€ (20% commission)', () => {
    const split = calculateSplit(49, 'lawyer');
    if (split.commissionEuros !== 9.8) throw new Error(`Commission: expected 9.8€, got ${split.commissionEuros}€`);
    if (split.providerEuros !== 39.2) throw new Error(`Provider: expected 39.2€, got ${split.providerEuros}€`);
    if (!split.isValid) throw new Error('Split should be valid');
  }));
  
  results.push(await runTest('Répartition expat 19€ (20% commission)', () => {
    const split = calculateSplit(19, 'expat');
    if (split.commissionEuros !== 3.8) throw new Error(`Commission: expected 3.8€, got ${split.commissionEuros}€`);
    if (split.providerEuros !== 15.2) throw new Error(`Provider: expected 15.2€, got ${split.providerEuros}€`);
    if (!split.isValid) throw new Error('Split should be valid');
  }));
  
  results.push(await runTest('Validation répartition cohérente', () => {
    const result = validateSplit(49, 9.8, 39.2);
    if (!result.valid) throw new Error(result.error);
  }));
  
  results.push(await runTest('Détection répartition incohérente', () => {
    const result = validateSplit(49, 10, 40); // 50€ au total au lieu de 49€
    if (result.valid) throw new Error('Should detect incoherent split');
  }));
  ...
  results.push(await runTest('Format 49€', () => {
    const formatted = formatEuros(49);
  ...
  results.push(await runTest('Format 49.99€', () => {
    const formatted = formatEuros(49.99);
  ...
  results.push(await runTest('Montant normal non suspect', () => {
    const result = isSuspiciousAmount(49, 'lawyer');
  ...
  results.push(await runTest('Montant avec trop de décimales suspect', () => {
    const result = isSuspiciousAmount(49.99999, 'lawyer');
    if (!result.suspicious) throw new Error('Should detect too many decimals');
  }));
  
  results.push(await runTest('Montant très différent suspect', () => {
    const result = isSuspiciousAmount(100, 'lawyer'); // 49€ attendu
  ...
  results.push(await runTest('Montant négatif rejeté', () => {
    const result = validateAmount(-10, 'lawyer');
    if (result.valid) throw new Error('Should reject negative amount');
  }));
  
  results.push(await runTest('Montant zéro rejeté', () => {
    const result = validateAmount(0, 'expat');
    if (result.valid) throw new Error('Should reject zero amount');
  }));
...
export async function runPerformanceTest(): Promise<void> {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    validateAmount(amount, type as 'lawyer' | 'expat');
    calculateSplit(amount, type as 'lawyer' | 'expat');
    eurosToCents(amount);
    centsToEuros(Math.round(amount * 100));
  }
...
export async function runIntegrationTest(): Promise<void> {
  ...
  const validation = validateAmount(49, 'lawyer');
  if (!validation.valid) {
  ...
  console.log(`     • Total: ${formatEuros(split.totalEuros)}`);
  console.log(`     • Commission: ${formatEuros(split.commissionEuros)}`);
  console.log(`     • Prestataire: ${formatEuros(split.providerEuros)}`);
  ...
  const fraudCheck = isSuspiciousAmount(49, 'lawyer');
