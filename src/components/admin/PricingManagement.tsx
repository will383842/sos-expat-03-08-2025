...
/* ================================
   Données « Avis » (FR) + avatars + fallback
   ================================ */
...
const OptimizedHomePage: React.FC = () => {
  ...
  // === EFFECTIVE PRICE (overrides inclus) ===
  const currencySymbol = selectedCurrency === 'eur' ? '€' : '$';
  const effLawyer = pricing ? getEffectivePrice(pricing, 'lawyer', selectedCurrency) : null;
  const effExpat  = pricing ? getEffectivePrice(pricing, 'expat', selectedCurrency)  : null;
        ...
        <section className="py-32 bg-gradient-to-b from-gray-950 to-gray-900 relative overflow-hidden" aria-labelledby="pricing-title">
          ...
          {(() => {
            ...
            interface OfferCardProps {
              ...
              borderColor: string;
              currency?: 'eur' | 'usd';
              /** si fourni, on rend ce bloc prix custom (override, etc.) */
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex-1 flex flex-col">
                    ...
                    {/* Bloc prix : custom (override) ou standard */}
                    {customPrice ? (
            ...
            return (
              <>
                ...
                <div className="relative z-10 max-w-7xl mx-auto px-6">
                  ...
                  ) : (pricingError || !pricing || !effLawyer || !effExpat) ? (
                    renderStaticPricing()
                  ) : (
                    <>
                      {/* Sélecteur de devise */}
                      ...
                      {/* Grid avec prix dynamiques (overrides inclus via getEffectivePrice) */}
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
                        {/* Card Expatrié */}
                        <OfferCard
                          badge="Offre Expatrié"
                          title="1 échange avec un expatrié expérimenté"
                          minutes={effExpat.price.duration}
                            ...
                            effExpat.override ? (
                              <div className="flex items-end gap-2">
                                ...
                                <span className="line-through text-gray-500">
                                  {currencySymbol}{effExpat.standard.totalAmount.toFixed(2)}
                                </span>
                                {/* Prix override */}
                                <span className="text-5xl font-black text-gray-900 leading-none">
                                  {currencySymbol}{effExpat.price.totalAmount.toFixed(2)}
                                </span>
                                {/* Badge label override si présent */}
                                {effExpat.override.label && (
                        ...
                        <OfferCard
                          badge="Offre Avocat"
                          title="1 consultation avec un avocat qualifié"
                          minutes={effLawyer.price.duration}
                          ...
                          customPrice={
                            effLawyer.override ? (
                              <div className="flex items-end gap-2">
                                <span className="line-through text-gray-500">
                                  {currencySymbol}{effLawyer.standard.totalAmount.toFixed(2)}
                                </span>
                                <span className="text-5xl font-black text-gray-900 leading-none">
                                  {currencySymbol}{effLawyer.price.totalAmount.toFixed(2)}
                                </span>
                                {effLawyer.override.label && (
                                  <span className="text-xs px-2 py-1 bg-red-100 text-red-700 rounded-full">
                                    {effLawyer.override.label}
                                  </span>
                                )}
                      ...
                      <div className="text-center mt-6">
                        <span className="text-xs text-white/60 bg-white/10 px-3 py-1 rounded-full">
                          💰 Tarifs mis à jour depuis la console admin (overrides inclus)
                        </span>
                      </div>
