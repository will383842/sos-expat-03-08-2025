...
const AdminExpats: React.FC = () => {
  ...
  const buildBaseQuery = useCallback(
    (opts?: { after?: QueryDocumentSnapshot<DocumentData> | null }) => {
      const base = [where("serviceType", "==", "expat_call")] as any[];
  ...
  const loadCount = useCallback(async () => {
    try {
      const cQuery = fsQuery(collection(db, "sos_profiles"), where("serviceType", "==", "expat_call"));
  ...
  const onBulk = async (action: "approve" | "reject" | "activate" | "suspend") => {
    ...
    const ops = ids.map(async (id) => {
      ...
      await updateDoc(doc(db, "sos_profiles", id), updates);
    });
  ...
  const handleStatusChange = async (id: string, status: ExpatStatus) => {
    await updateDoc(doc(db, "sos_profiles", id), { status, updatedAt: new Date() });
